...
class Header extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    const dataIndex = this.props.navData.findIndex(function(value, index, arr) {
      return value.href === nextProps.pathname;
    });
    this.setState({ currentIndex: dataIndex });
  }
  render() {
            ...
            <div className="container">
              <ul className="nav-cells">
                {this.props.navData.map((value, index) => {
                  return (
                    <li key={index} className={this.getItemCssClasses(index)}>
